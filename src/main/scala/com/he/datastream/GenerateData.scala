...
object GenerateData {
  private val delaySeconds = 60
  private val format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")

  def main(args: Array[String]): Unit = {
    val properties = new Properties()
    properties.put("bootstrap.servers","192.168.40.102:9093")
      ...
      if (i % 100 == 0){
        val thread = new Thread(new Runnable {
          override def run(): Unit = {
            ...
            println("Delay data to test2-> createTime:"+createTime+",sendTime:"+format.format(new Date()))
